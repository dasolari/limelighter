...
if (messageContainer && messageForm && messageContainer.getAttribute('type') === 'user-chat') {
  const socket = io();
  ...
  if (messages) {
    messages.forEach((elem) => {
      appendMessage(elem.userId, elem.content, new Date(elem.date));
    });
    messages = {};
  }
  ...
  socket.on('chat-message', (data) => {
    appendMessage(data.userId, data.message, new Date());
  });
  ...
  messageForm.addEventListener('submit', (e) => {
    ...
    appendMessage(parsedUser.id, message, new Date());
    socket.emit('send-user-chat-message', { message, user });
    messagesArray.push({
      ...
      chatId: messageForm.elements.namedItem('chatId').value,
      date: new Date(),
    });
    messageInput.value = '';
  });
  ...
  goBackContainer.addEventListener('click', () => {
    if (messagesArray) {
      ...
      cId.type = 'number';
      cId.name = 'chatId';
      cId.value = parsedUser.chatId;
      saveMessagesForm.appendChild(msg);
      saveMessagesForm.appendChild(cId);
      document.body.appendChild(saveMessagesForm);
      saveMessagesForm.submit();
    }
  });

  function appendMessage(senderId, message, date) {
